...
# Эйлер считает, что на всём маленьком отрезке времени скорость пости не меняется 
# RK45 два приближения 4-го порядка и 5-го порядка 
# solve_ivp решает проблему Коши, IVP - Initial Value Proplem
...
plot_many(y_axis=t, funcs=(x1, x2),
          title=f"Метод Рунге_Кутта: X'(t)=A·X(t), шаг={step}",
          xlab="t, c", ylab="x(t)")
